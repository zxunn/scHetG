...
def add_geneset_human_lung(dataset, gene_embeddings, gene_names, method):
    ...
    ratio = cal_ratio(gene_embeddings, df_plot)
...
df_plot, dim_labels = add_geneset_human_lung("human lung", gene_embeddings, gene_names, "normalized data")
...
df_plot, dim_labels = add_geneset_human_lung("human lung", gene_embeddings, gene_names, "SIMBA")
df_plot.insert(loc=4, column='gene', value=LabelEncoder().fit_transform(df_plot.Geneset.values).astype(np.int_))
...
gene_embeddings = gene_embeddings[data_genes_indices]
gene_names = gene_names[data_genes_indices]
df_plot, dim_labels = add_geneset_human_lung("human lung", gene_embeddings, gene_names, "scHetG")
df_plot.insert(loc=4, column='gene', value=LabelEncoder().fit_transform(df_plot.Geneset.values).astype(np.int_))
