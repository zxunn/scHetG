...
def add_geneset_human_lung(dataset, gene_embeddings, gene_names, method):
    ...
    selected_ct = np.array(['lung macrophage', 'lung endothelial cell', 'lung secretory cell','B cell'], dtype=object)
...
if __name__ == "__main__":
    if os.path.exists("./gene&cell_emb_human_lung_scHetG"):
        data = sc.read_h5ad("./gene&cell_emb_human_lung_scHetG")  # gene embeddings obtained by scHetG
    else:
        from scHetG_master import train_scHetG
        from scHetG_master import preprocess
        data = sc.read_h5ad("../../datasets/human_lung/human_lung_marker.h5ad")
        data.obs['batch'].replace('muc3843', 'batch1', inplace=True)
        data.obs['batch'].replace('muc4658', 'batch2', inplace=True)
        data.obs['batch'].replace('muc5103', 'batch3', inplace=True)
        data.obs['batch'].replace('muc5104', 'batch4', inplace=True)
        ...
        data.write_h5ad("./gene&cell_emb_human_lung_scHetG")
    if os.path.exists("./gene&cell_emb_human_lung_SIMBA"):
        SIMBA_data = sc.read_h5ad("../../gene&cell_emb_human_lung_SIMBA")  # gene embeddings obtained by SIMBA
    else:
        ...
        SIMBA_data= simba_func(adata1, adata2,adata3, adata4, dataset)
        SIMBA_data.write_h5ad("./gene&cell_emb_human_lung_SIMBA")
    ...
    #Ensure that different methods use the same genes
    ...
    df_plot, dim_labels, L1,celltype = add_geneset_human_lung("human lung", gene_embeddings, gene_names, "normalized data")
    ...
    df_plot, dim_labels, L2,celltype = add_geneset_human_lung("human lung", gene_embeddings, gene_names, "SIMBA")
    df_plot.insert(loc=4, column='gene', value=LabelEncoder().fit_transform(df_plot.Geneset.values).astype(np.int_))
    ...
    gene_embeddings = gene_embeddings[data_genes_indices]
    gene_names = gene_names[data_genes_indices]
    df_plot, dim_labels, L3,celltype = add_geneset_human_lung("human lung", gene_embeddings, gene_names, "scHetG")
    df_plot.insert(loc=4, column='gene', value=LabelEncoder().fit_transform(df_plot.Geneset.values).astype(np.int_))
    ...
    plt.savefig('figure_co/human_lung.png', format='png', dpi=900)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('heat_graph/human_lung.png', format='png', dpi=900)
