...
for dataset in ['mouse_pancreas', 'mouse_atlas','human_pancreas','human_pancreas_2','human_lung','human_heart','mouse_brain']:
    ...
    adata_corrd = ad.AnnData(corrd, obs=adata_c.obs, dtype='float64')
    adata_corrd.obsm['X_pca_harmony'] = adata_c.obsm['X_pca_harmony']
    adata_corrd.obs['celltype'] = np.array(adata_c.obs['celltype'])
    adata_corrd.obs['batch'] = np.array(adata_c.obs['batch'])
    ...
    sc.pp.neighbors(adata_corrd,use_rep='X_pca_harmony')
    sc.tl.louvain(adata_corrd, resolution=0.5)

    #adata_corrd.write_h5ad("Harmony_scanpy"+dataset + ".h5ad")
