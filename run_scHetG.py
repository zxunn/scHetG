...
import pandas as pd
from train.train_scHetG import train_scHetG
...
from train.utils import louvain, calculate_metric, calculate_ber
import scib
...
for dataset in ['mouse pancreas', 'mouse atlas','human pancreas','human pancreas 2','human lung','human heart','mouse brain']:
    ...
    if dataset == 'mouse pancreas':
        ...
        adata = sc.read_h5ad("./datasets/simulation/simulation.h5ad")
        adata.obs.rename(columns={'Batch': 'batch'}, inplace=True)
        adata.obs.rename(columns={'Group': 'celltype'}, inplace=True)
        batch_name = np.array(['batch1', 'batch2', 'batch3'])
        adata.obs['batch'].replace('Batch1', 'batch1', inplace=True)
        adata.obs['batch'].replace('Batch2', 'batch2', inplace=True)
        adata.obs['batch'].replace('Batch3', 'batch3', inplace=True)

    elif dataset == 'simulation batch1&2':
        adata = sc.read_h5ad("./datasets/simulation/simulation_batch1&2.h5ad")
        batch_name = np.array(['batch1', 'batch2'])
    start = time()
    ...
    if common_genes:
        ...
        sc.pp.highly_variable_genes(adata2, n_top_genes=3000, flavor='seurat_v3')
        highly_variable1 = adata1.var['highly_variable']
        highly_variable2 = adata2.var['highly_variable']
        highly_variable = [highly_variable1, highly_variable2, highly_variable1|highly_variable2]
        adata1[:, (~highly_variable1) & highly_variable2].X = np.zeros(adata1[:, (~highly_variable1) & highly_variable2].X.shape)
        adata2[:, (~highly_variable2) & highly_variable1].X = np.zeros(adata2[:, (~highly_variable2) & highly_variable1].X.shape)
        adata1 = preprocess(adata1, common_genes)
        adata2 = preprocess(adata2, common_genes)
        adata1 = adata1[:, highly_variable[-1]]
        adata2 = adata2[:, highly_variable[-1]]
        adata = ad.concat([adata1, adata2], merge="same")
        batch_name = np.array(['batch1', 'batch2'])
    ...
    #training
    adata = train_scHetG(adata, batch_name, n_clusters=n_clusters, cl_type='celltype',
                          feats_dim=64, lr=0.15, n_layers=2,
                          drop_out=0.1, decoder='ZINB', gamma=1,
                          log_interval=10, iteration=200, early_stop_epoch=50,
                          sample_rate=0.1, learnable_w=True, highly_variable=highly_variable, common_genes=common_genes,
    ...
    sc.tl.umap(adata)
    sc.pl.umap(adata, color=['batch', 'celltype', 'louvain'], save="scHetG_"+dataset+".pdf")
    adata.write_h5ad("scHetG_" + dataset + ".h5ad")
